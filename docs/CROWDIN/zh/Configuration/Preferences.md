...
- Necessary to be able to [export settings](../Usage/ExportImportSettings.md) as they are encrypted from version 2.7.
...
- _How frequently SMBs will be given in min_ is a restriction for SMB to be delivered only every 4 min by default. This value prevents the system from issuing SMB too often (for example in case of a temp target being set). You should not change this setting unless you know exactly about consequences.
...
- To put it simply: The algorithm "knows" how your BGs _should_ behave when affected by the current dose of insulin etc.
